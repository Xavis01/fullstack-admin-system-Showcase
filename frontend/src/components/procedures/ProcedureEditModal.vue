        ...
        <transition name="modal">
          <div v-if="isOpen" class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 md:p-8 z-10">
            ...
            <div class="flex items-center gap-3 mb-6">
              ...
              <h3 class="text-xl font-bold text-neutral-800">Editar Procedimento</h3>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <input v-model="nome" type="text"
                  class="w-full border border-neutral-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition" />
              </div>

              <!-- File (optional) -->
              <div>
                <label class="block text-sm font-semibold text-neutral-600 mb-1.5">
                  Substituir arquivo <span class="text-neutral-400 font-normal">(opcional)</span>
                </label>
                <div class="border-2 border-dashed border-neutral-300 rounded-xl p-4 text-center cursor-pointer hover:border-blue-400 transition"
                  @click="$refs.fileInput.click()"
                  @dragover.prevent
                  @drop.prevent="onDrop">
                  <template v-if="!file">
                    <p class="text-xs text-neutral-500 font-medium">Arquivo atual:</p>
                    <p class="text-sm text-neutral-700 truncate mt-0.5">{{ procedure?.original_filename }}</p>
                    <p class="text-xs text-neutral-400 mt-2">Clique ou arraste para substituir</p>
                  </template>
                  <template v-else>
            ...
            <div class="flex gap-3 mt-6">
              ...
              <button @click="submit" :disabled="loading"
                class="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold transition text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
...
import { Pencil, FileCheck } from 'lucide-vue-next'
...
watch(() => props.isOpen, (val) => {
  if (val && props.procedure) {
    nome.value = props.procedure.nome
    file.value = null
  }
})
