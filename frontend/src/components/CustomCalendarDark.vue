<template>
  <div class="relative w-full" ref="calendarContainer">
    <!-- Input Display com estilo do LoginView -->
    <div class="relative items-center">
      <input
        ...
        @focus="openCalendar"
        class="input w-full bg-[#333] text-white border border-gray-600 rounded-lg h-[44px] px-3 pt-5 pb-1 leading-tight transition duration-200 outline-none"
        :class="{
          'border-red-500': isOpen,
          'hover:border-gray-500': !isOpen
        }"
      />
      <span class="input-label">{{ label }}</span>
      <Calendar 
        class="w-5 h-5 text-gray-400 absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer hover:text-red-500 transition" 
    ...
    <Teleport to="body">
      ...
      >
        <div
          v-if="isOpen"
          ref="dropdownRef"
          class="fixed z-50 bg-[#1a1a1a] border-2 border-gray-700 rounded-lg shadow-lg p-4 w-64"
          :style="dropdownStyle"
        >
          <!-- Header -->
          <div class="flex justify-between items-center mb-4">
            <button @click="prevMonth" type="button" class="p-1 hover:bg-gray-800 rounded-full transition">
              <ChevronLeft class="w-5 h-5 text-gray-400" />
            </button>
            <span class="font-bold text-white capitalize">
              {{ currentMonthName }} {{ currentYear }}
            </span>
            <button @click="nextMonth" type="button" class="p-1 hover:bg-gray-800 rounded-full transition">
          ...
          <div class="grid grid-cols-7 gap-1">
            ...
            <button
              ...
              class="h-8 w-8 rounded-full flex items-center justify-center text-sm transition duration-200 hover:bg-red-900/30 hover:text-red-400"
              :class="{
                'bg-red-600 text-white hover:bg-red-700 hover:text-white': isSelected(day),
                'text-gray-300': !isSelected(day),
                'font-bold border border-red-500': isToday(day)
              }"
            >
              {{ day }}
            </button>
...
<style scoped>
/* Input label animado igual ao LoginView */
.input-label {
