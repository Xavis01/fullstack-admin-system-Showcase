        ...
        <transition name="modal">
          <div v-if="isOpen"
            ...
            <div
              ...
              <h3 class="text-lg md:text-xl font-bold">FILTRAR PRODUÇÃO</h3>
            </div>
            ...
            <div class="p-4 sm:p-5 md:p-6">
              ...
              <div class="mb-4 md:mb-6 flex items-center cursor-pointer"
                @click="filters.hide_empty_stock = !filters.hide_empty_stock">
                <div
                  class="w-5 h-5 border-2 border-gray-300 rounded flex items-center justify-center transition-colors duration-200"
                  :class="{ 'bg-red-600 border-red-600': filters.hide_empty_stock, 'bg-white': !filters.hide_empty_stock }">
                  <Check v-if="filters.hide_empty_stock" class="w-3.5 h-3.5 text-white" stroke-width="3" />
                </div>
                <label class="ml-2 text-xs md:text-sm font-bold text-gray-700 cursor-pointer select-none">Ocultar
                  estoque
                  vazio</label>
              </div>
...
const clear = () => {
  filters.value = { num_lote: '', product_id: '', start_date: '', end_date: '', hide_empty_stock: false }
  emit('apply', filters.value)
}
</script>
