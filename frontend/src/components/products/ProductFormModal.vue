        ...
        <transition name="modal">
          <div v-if="isOpen"
            ...
            <div
              ...
              <h3 class="text-lg md:text-xl font-bold">{{ product ? 'EDITAR PRODUTO' : 'CADASTRO DE PRODUTO' }}</h3>
            </div>
            <!-- Body Omitted -->
            <div class="p-4 sm:p-6 md:p-8">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6">
                ...
                <div>
                  <label class="block text-gray-700 text-xs md:text-sm font-bold mb-2">Unidades por Caixa</label>
                  <input v-model="formData.unidades_por_caixa" type="number" placeholder="Digite a quantidade..."
                    class="w-full bg-gray-100 text-gray-700 border-2 border-gray-300 rounded-lg h-[44px] px-4 leading-tight hover:border-gray-400 focus:outline-none focus:border-red-500 focus:bg-white transition duration-200" />
                </div>
              </div>
...
watch(() => props.isOpen, (isOpen) => {
  if (isOpen) {
    if (props.product) {
      ...
      formData.value = { nome: '', unidades_por_caixa: '' }
    }
  }
})
...
const save = () => {
  ...
  if (!formData.value.nome || !formData.value.nome.trim()) {
    errors.push('Nome do Produto')
  }
  if (!formData.value.unidades_por_caixa || Number(formData.value.unidades_por_caixa) <= 0) {
    errors.push('Unidades por Caixa (deve ser maior que 0)')
  }
